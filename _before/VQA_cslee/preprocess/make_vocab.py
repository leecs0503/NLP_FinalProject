...
def make_quest_vocab(input_dir: str, output_dir: str, n_answers: int):
    ...
    print(
        f"Write Question Vocab Complete (Size = {len(quest_vocab)}, path = {quest_vocab_file_path})"
    )
...
def make_ans_vocab(input_dir: str, output_dir: str, n_answers: int):
    ...
    print("Write Done")
    print(
        f"The number of total words of answer: {len(answer_vocab)}, we keep {n_answers} answers by occurency"
    )
    print(
        f"The minimum occurency of top {n_answers} is {answer_vocab[n_answers-1:n_answers][0][1]}"
    )
